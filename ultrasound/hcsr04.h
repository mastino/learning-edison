...
#define CM_DIV   58   
#define IN_DIV   148  
#define PULSE_T  500  // in milli sec
...
public:
  ...
  long get_inches();
  long get_cmeters();
  bool wait_above_threshold(long threshold, bool is_inches);
  bool wait_below_threshold(long threshold, bool is_inches);

  void set_pins(int new_trig_pin, int new_echo_pin);
  void set_cm_div(long new_cm_div);
  void set_in_div(long new_in_div);
  void set_pulse_T(int new_pulse_T);
  void get_cm_div(long &out_cm_div);
  long get_cm_div();
  void get_in_div(long &out_in_div);
  long get_in_div();
...
private:
  ...
  long cm_div, in_div;     // divisors to get real units
